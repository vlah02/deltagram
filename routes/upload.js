    ...
    router.post('/', upload.array('zipfiles'), async (req, res) => {
        ...
        for (const file of req.files) {
            ...
            try {
                const followersPath = `${extractPath}/connections/followers_and_following/followers_1.json`;
                const followingPath = `${extractPath}/connections/followers_and_following/following.json`;
                const followersList = extractUsers(followersPath);
                const followingList = extractUsers(followingPath);
                followersCount = followersList.length;
                followingCount = followingList.length;
            } catch (e) {
                console.warn('Error extracting followers/following count', e);
            }

            db.run(
                'INSERT INTO uploads (user_id, filename, created_at, file_hash, followers_count, following_count) VALUES (?, ?, ?, ?, ?, ?)',
                [userId, uploadId, realDate, hash, followersCount, followingCount],
                (err) => {
